...
<template>
  <div class="space-y-10">
    ...
    <AppLoading
      v-if="pending"
      text="Loading platform order audit..."
    />
    ...
    >
      ...
      <p class="mt-2 text-sm text-[var(--shopizz-obsidian)]/55">
        Unable to locate this order record in the platform database.
      </p>
      <NuxtLink to="/admin/orders" class="mt-6 inline-block">
        <AppButton size="sm">
          Return to orders
        </AppButton>
      </NuxtLink>
    ...
    <div v-else class="space-y-8">
      ...
      <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <p class="mt-1 text-xs text-[var(--shopizz-obsidian)]/45">
            Placed on {{ formatDate(order.createdAt) }} • Buyer: {{ order.buyer?.name }} ({{ order.buyer?.email }})
          </p>
        </div>
      ...
      <div class="grid gap-8 lg:grid-cols-12">
        <!-- Left: All Multi-Vendor Items Ordered (7 cols) -->
        <div class="space-y-6 lg:col-span-7">
          <OrderItems :items="order.items || []" />
        </div>

        <!-- Right: Address & Financial Summary (5 cols) -->
