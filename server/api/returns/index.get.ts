...
export default defineEventHandler(async (event) => {
  ...
  const rows = await db
    ...
    .leftJoin(users, eq(returnRequests.ownerId, users.id))
    .where(eq(returnRequests.buyerId, user.id))
  ...
  for (const r of rows) {
    if (!returnMap.has(r.id)) {
      returnMap.set(r.id, {
        ...
        ownerName: r.ownerName || 'Studio Maker',
        reason: r.reason,
  ...
  return {
    success: true,
    returns: Array.from(returnMap.values()),
  }
})
