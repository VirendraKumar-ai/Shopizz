...
export default defineEventHandler(async (event) => {
  ...
  if (status === 'APPROVED') {
    ...
    // Mark order status and payment as REFUNDED
    await db
      .update(orders)
      .set({
        status: 'REFUNDED',
    ...
    await db.insert(orderStatusHistory).values({
      ...
      status: 'REFUNDED',
      message: `Admin approved refund for Return #${returnReq.returnNumber} (₹${(returnReq.refundAmount / 100).toFixed(2)}). Reference: ${finalTxnId}`,
      updatedBy: user.id,
    })
  ...
  try {
    ...
    if (status === 'APPROVED') {
      await createNotification({
        ...
        message: `Your return request #${returnReq.returnNumber} for Order #${orderNumber} has been approved.`,
      ...
      if (buyerEmail) {
        await sendReturnApprovedEmail(
          ...
          orderNumber,
          sellerNotes || 'Please ensure original packaging is intact. Courier pickup will be scheduled.'
        )
      }
    } else if (status === 'ITEM_RECEIVED') {
      await createNotification({
        ...
        message: `Returned piece for #${returnReq.returnNumber} has arrived and is undergoing inspection.`,
        link: `/account/orders/${returnReq.orderId}`,
      })
    } else if (status === 'REFUNDED') {
      ...
      await createNotification({
        ...
        message: `₹${refundAmountRupees.toLocaleString('en-IN')} refunded for Return #${returnReq.returnNumber}.`,
        link: `/account/orders/${returnReq.orderId}`,
      })
      ...
      await createNotification({
        ...
        message: `Return #${returnReq.returnNumber} was not approved. Reason: ${rejectionReason}`,
        link: `/account/orders/${returnReq.orderId}`,
      })
    ...
    console.error('[Admin Return Status Update Notification Error]:', err)
  }
